...
class game:
    ...
    def update_status_move(self):
        ...
        for i in range(min(3,len(self.BULLETS0))):
            index = argmin(tmpdist0)
            tmpbul0.append(self.BULLETS0[index])
            tmpdist0[index] = 10000000

        for i in range(min(3,len(self.BULLETS1))):
            index = argmin(tmpdist1)
