...
def training(loading, check_epoisode):
    ...
    with tf.Session() as sess:
        ...
        rotate_mainDQN1 = DQN(sess, rotate_input_size, rotate_output_size, name="rotatemain1")
        rotate_targetDQN1 = DQN(sess, rotate_input_size, rotate_output_size, name="rotatetarget1")

        
        if loading:
            loadDQN(move_mainDQN0,'./CheckPoints/DQN_m0-',check_epoisode)
            loadDQN(move_targetDQN0,'./CheckPoints/DQN_m0-',check_epoisode)
            loadDQN(move_mainDQN1,'./CheckPoints/DQN_m1-',check_epoisode)
            loadDQN(move_targetDQN1,'./CheckPoints/DQN_m1-',check_epoisode)
            
            loadDQN(rotate_mainDQN0,'./CheckPoints/DQN_r0-',check_epoisode)
            loadDQN(rotate_targetDQN0,'./CheckPoints/DQN_r0-',check_epoisode)
            loadDQN(rotate_mainDQN1,'./CheckPoints/DQN_r1-',check_epoisode)
            loadDQN(rotate_targetDQN1,'./CheckPoints/DQN_r1-',check_epoisode)
        ...
        for episode in range(check_epoisode, check_epoisode+max_episodes):
            ...
            while not done:
                ...
                if np.random.rand(1) < e:
                    ar0 = np.random.randint(rotate_output_size, size=1)
                else:
                    ar0 = np.argmax(rotate_mainDQN0.predict(sr0))

                if np.random.rand(1) < e:
                    ar1 = np.random.randint(rotate_output_size, size=1)
                else:
                    ar1 = np.argmax(rotate_mainDQN1.predict(sr1))

                # move DQN e-greedy
                if np.random.rand(1) < e:
                    am0 = np.random.randint(move_input_size, size=1)
                else:
                    am0 = np.argmax(move_mainDQN0.predict(sm0))

                if np.random.rand(1) < e:
                    am1 = np.random.randint(move_input_size, size=1)
                else:
                    am1 = np.argmax(move_mainDQN1.predict(sm1))
                done, win, nsm0, nsm1, nsr0, nsr1 = env.step(ar0, am0, ar1, am1)
                rm0, rm1 = step_count, step_count
                rr0, rr1 = step_count, step_count
                ...
                if done:
                    if win == 0:
                        for i in range(min(episode,5)):
                            replay_buffer_m[len(replay_buffer_m) - 2*i+1][2] = -step_count
                    else :
                        for i in range(min(episode,5)):
                            replay_buffer_m[len(replay_buffer_m) - 2*i  ][2] = -step_count
